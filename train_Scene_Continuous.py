...
# Input
parser.add_argument("--pretrained", type=str, default="/media/fhy/My_Passport/ZKX/Bi3D-master/src/log_tree/checkpoint_000008.pth")
...
inital_learning_rate = optimizer.param_groups[0]["lr"]
...
def train():
    ...
    for epoch_idx in range(start_epoch, start_epoch + args.epochs):
        ...
        adjust_learning_rate(optimizer, epoch_idx, inital_learning_rate, args.lrepochs)  # 训练到一定次数时，需要调整学习率。
        # 每次从数据集中读取一批图像进行训练。
